...
- **Phase 1 — Aider routing improvements**: Task-size classification (`classify_task_size()`) routing small edits → Aider, large → JCode, research → Hermes with availability detection. Enhanced handoff with acceptance criteria, exact writable paths, test command, risks, and stop conditions. Execution evidence collection (git diff, test results, token/provider usage, remaining risks). CLI commands: `guardian aider classify`, `guardian aider evidence`. 18 new tests (24 total).
- **Phase 2 — JCode safe adapter**: Binary/version detection (`jcode_status()`) with 15s timeout and safe unavailable reporting. Dry-run handoff (`create_jcode_handoff()`) with compact context, writable paths, and 11 enforced restrictions documented. Safe command construction (`build_jcode_command()`) with `--dry-run` default, `JCODE_NO_TELEMETRY=1`, and protected-path filtering. CLI commands: `guardian jcode status`, `guardian jcode prepare`, `guardian jcode command`. 19 new tests.
- **Phase 3 — JCode controlled execution**: Explicit user opt-in (`jcode_opt_in()`/`jcode_is_opted_in()`) with consent stored in `.agent/jcode_consent.json`. Sandbox/worktree execution via `execute_jcode_in_sandbox()` with configurable timeout (10-3600s), stdout/stderr/exit-code capture, and process cleanup. Diff evidence via `_git_diff_in_sandbox()` with +/− change counts. Test result capture after JCode completes. Out-of-scope rejection via `_validate_out_of_scope()` — flags changes outside allowed writable paths. Structured result with `approved: False` always — model/user must make final call. CLI commands: `guardian jcode opt-in`, `guardian jcode run`. 10 new tests (29 total).
- **Phase 4 — JCode bounded parallel work**: `jcode_parallel_run()` orchestrates up to 2 workers via `ThreadPoolExecutor`. `_check_path_conflicts()` validates independent writable paths between packages. `_lock_writable_paths()`/`_unlock_writable_paths()` with JSON lock files, 1-hour TTL, stale release. `_notify_workers()` copies diff info into pending workers' sandboxes. Stop conditions: timeout, exit code, out-of-scope changes, test failure, emergency stop. CLI: `guardian jcode parallel-run`. 18 new tests (47 total).
- **Priority 3 — True per-ticket concurrency**: `process_ready_tickets()` upgraded from sequential for-loop to `ThreadPoolExecutor` with `max_workers` parameter (default 4, range 1-16). CLI `--max-workers` added to `guardian executor run`. Timing-based test proves overlapping execution.
...
| Background supervisor and worker dispatch | **Implemented** | `process_ready_tickets()` upgraded to `ThreadPoolExecutor` with `max_workers` parameter (1-16). True parallel ticket execution verified with timing-based tests. CLI `--max-workers` exposed. |
